...
export function createQwikCity(opts: QwikCityBunOptions) {
  ...
  async function router(request: Request) {
    try {
      ...
      const serverRequestEv: ServerRequestEvent<Response> = {
        ...
        getClientConn: () => {
          return opts.getClientConn ? opts.getClientConn(request) : { ip: undefined };
        },
      };
  ...
  const staticFile = (request: Request) => {
    try {
      const url = new URL(request.url);

      if (isStaticPath(request.method || 'GET', url)) {
        const { filePath, content } = openStaticFile(url);
